...
export class TournamentWriteService implements ITournamentWriteService {
  ...
  async create(t: CreateTournamentDto): Promise<Result<TournamentDto>> {
    ...
    const created:Tournament = await this.tournamentWriteRepo.create(newTournament);
  ...
  async update(id: number, fields: TournamentDto): Promise<Result<void>> {
    const t = await this.tournamentReadRepo.findById(id);
    if(t.tournamentId === 0)
    {
    ...
    const res = await this.tournamentWriteRepo.update(id, updatedTournament)
