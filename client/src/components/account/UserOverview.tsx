...
type UserOverviewState =
  ...
  | { status: "loaded"; user: UserDto };

export default function UserOverview({ id }: { id?: number }) {
  const { user: authUser } = useAuth();
  const [state, setState] = useState<UserOverviewState>({ status: "loading" });
  const [open, setOpen] = useState(false);
  const [image, setImage] = useState("");

  const resolvedId = id ?? authUser?.id ?? 0;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const res = await usersApi.getById(resolvedId);
      ...
      setState({ status: "loaded", user: res.data });
    }
  ...
  return (
    <div className="w-full flex justify-center items-center py-10 bg-primary border-bgprimary/80 border-3 shadow-[0_0_25px] shadow-bgprimary/80 rounded-xl min-w-110">
      <div className="grid lg:grid-cols-2 sm:grid-cols-1 items-center lg:gap-10 sm:gap-1">
        <div className="relative w-40 h-40 justify-self-center">
          <img
            draggable={false}
            alt="Profile"
            className="w-40 h-40 object-cover rounded-full image-rendering-auto justify-self-center cursor-pointer"
            onClick={() => {
              setOpen(true);
              setImage(userInfo.profilePicture ? userInfo.profilePicture : avatarPlaceholder);
            }}
            src={userInfo.profilePicture ? userInfo.profilePicture : avatarPlaceholder}
          />
        ...
        <div className="grid sm:grid-rows-3 lg:grid-rows-6 sm:grid-cols-2 lg:grid-cols-1 sm:col-1 lg:col-2 gap-1 mt-2">
          ...
          <div className="sm:col-2 lg:col-1 sm:row-1 lg:row-2">
            <span className="text-bgprimary">
              {userInfo.gamerTag} <RoleBadge role={userInfo.role} />
            </span>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-85 rounded-3xl p-6">
            <img
              draggable={false}
              alt="Profile"
              className="w-64 h-64 object-cover image-rendering-auto justify-self-center cursor-pointer"
